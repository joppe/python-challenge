...
for pos in range(3, len(source) - 3):
    c1 = is_upper(source[pos - 3])
    c2 = is_upper(source[pos - 2])
    c3 = is_upper(source[pos - 1])
    c4 = is_lower(source[pos])
    c5 = is_upper(source[pos + 1])
    c6 = is_upper(source[pos + 2])
    c7 = is_upper(source[pos + 3])

    if pos > 3:
        c0 = is_lower(source[pos - 4])
    else:
        c0 = True

    if pos < len(source) - 5:
        c8 = is_lower(source[pos + 4])
    else:
        c8 = True

    if c0 and c1 and c2 and c3 and c4 and c5 and c6 and c7 and c8:
        out += source[pos]
